...
const CartItem = ({ id, img, title, price, quantity }) => {
  const {remove,changeQNT} = useCartContext()
  return (
    <div className="d-flex justify-content-between align-items-center mt-3">
      ...
      <div className="pt-2 pt-sm-0 ps-sm-3 mx-auto mx-sm-0 text-center text-sm-end">
        ...
        <input className="form-control" type="number" value={quantity} onChange={(e) => {changeQNT({id:id,quantity:+e.target.value})}} />
        <button className="btn btn-link text-danger pe-0" type="button">
          <RiCloseCircleLine className="ms-1"/>
          <span className="fs-sm" onClick={()=>remove(id)}>حذف</span>
        </button>
      </div>
    </div>
  );
